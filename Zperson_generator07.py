...
def write_file(fileName, content, addNewline=True, overWrite=False, subPath=""):
    if overWrite:
...
def generate_person(fullyRandom=False, specifiedData = {}, writeFile=True):
    ...
    if writeFile:
        #klopot z ponownym wykorzystaniem obiekut personDataList
        fileName = personDataDictio["Name"].lower() + personDataDictio["Surname"].lower() + ".txt"
        someName = personDataDictio["Name"].lower() + personDataDictio["Surname"].lower() + "2.txt" #can write twice, why??
        write_file(fileName, personDataList, addNewline=True, overWrite=True, subPath="personData")    
        # write_file("TTTT.txt", ['hhhm'], addNewline=True, overWrite=True, subPath="")    
        write_file("AllPersonData", personDataList, addNewline=True, overWrite=False, subPath="personData") #can write twice, why??
    # print('hmmm', personDataList)
    for item in personDataList:
        print(item)
        # print(33)
    
    #personDataList = list(personDataDictio.items())    
